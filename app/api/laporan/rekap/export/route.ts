...
export async function GET(req: Request) {
  ...
  try {
    const backendRes = await fetch(`${API_URL}/api/v1/laporan/rekap/export${qs}`, {
      headers: {
    ...
    const filename = backendRes.headers.get("content-disposition")?.match(/filename="?([^";]+)"?/)?.[1] || "laporan-rekap.xlsx"
