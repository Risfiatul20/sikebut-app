...
const pengadaanStore: RkbmdPengadaanItem[] = [...INITIAL_RKBMD_PENGADAAN]
...
export async function GET(req: Request) {
  ...
  const sort_by = searchParams.get("sort_by") ?? "id_pengadaan"
  const sort_direction = searchParams.get("sort_direction") ?? "desc"

  try {
    const backendUrl = `${process.env.API_URL || "http://127.0.0.1:8000"}/api/v1/rkbmd-pengadaan?${searchParams.toString()}`
  ...
  const filtered = pengadaanStore.filter((item) => {
    ...
    if (search) {
      ...
      const matchSub = item.nama_sub_giat_nama_sub_giat?.toLowerCase().includes(search) ?? false
      if (!matchNama && !matchFikasi && !matchSkpd && !matchSub) return false
    }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    try {
      const backendUrl = `${process.env.API_URL || "http://127.0.0.1:8000"}/api/v1/import/rkbmd-pengadaan`
      ...
      const mockImportId = `rkbmd-pengadaan-${Date.now()}`
      
      const mockNewItem: RkbmdPengadaanItem = {
        id_pengadaan: Date.now(),
        ...
        nama_sub_giat_nama_sub_giat: "Hasil Impor Berkas RKBMD Pengadaan",
        kode_fikasi: "1.3.2.01.09",
        nama_barang: `Barang Hasil Impor Excel (${(file as File).name})`,
        jumlah_barang: 10,
        satuan: "Unit",
        jumlah_maksimum: 15,
        cara_pemenuhan: "Pembelian Langsung",
        keterangan: "Data diimpor melalui simulasi unggah berkas",
        nm_status: "Disetujui Penelaah",
        periode: 2026,
        created_at: new Date().toISOString(),
      }

      pengadaanStore.unshift(mockNewItem)
