    ...
    selected.forEach((pagu, id) => {
      ...
      if (source) {
        items.push({
          ...
          // Salin flag akun_indikator_rkbmd → acuan pertanyaan RKBMD per kode rekening
          is_belanja_pengadaan: Boolean(source.is_belanja_pengadaan),
          is_rkbmd_pengadaan: Boolean(source.is_rkbmd_pengadaan),
          is_rkbmd_pemeliharaan_rehab: Boolean(source.is_rkbmd_pemeliharaan_rehab),
          is_rkbmd_pemeliharaan_rutin: Boolean(source.is_rkbmd_pemeliharaan_rutin),
        })
      }
    })
    onSelect(items)
  ...
  const renderLevelCell = (row: (typeof LEVEL_ROWS)[number], level: SipdModalLevel) => {
    ...
    return (
      <div className="min-w-0 space-y-0.5">
        <span className="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
          {row.label}
        </span>
        <p className="font-mono text-[9px] text-slate-400 truncate">{String(level[f.kode] ?? "-")}</p>
        <p className="font-medium text-slate-900 dark:text-white text-xs leading-snug truncate" title={String(level[f.nama] ?? "")}>{String(level[f.nama] ?? "-")}</p>
      </div>
    )
  }
      ...
      <div className="w-full max-w-5xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/50 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-display text-base font-semibold text-slate-900 dark:text-white">Pagu Paket</h2>
              ...
              <section>
                ...
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 overflow-x-auto">
                  <table className="w-full text-xs whitespace-nowrap">
                    <thead>
                      <tr className="text-left text-[10px] uppercase tracking-wide text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                        <th className="font-semibold px-3 py-2.5 min-w-[240px]">Level</th>
                        <th className="font-semibold px-3 py-2.5 text-right min-w-[115px]">Pagu</th>
                        <th className="font-semibold px-3 py-2.5 text-right min-w-[135px]">Pagu Non Pengadaan</th>
                        <th className="font-semibold px-3 py-2.5 text-right min-w-[125px]">Pagu Pengadaan</th>
                        <th className="font-semibold px-3 py-2.5 text-right min-w-[135px]">Pagu Paket (Kebutuhan)</th>
                        <th className="font-semibold px-3 py-2.5 text-right min-w-[115px]">Sisa</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {LEVEL_ROWS.map((row, idx) => {
                        ...
                        return (
                          <tr key={row.key} className={`transition-colors ${idx % 2 === 1 ? "bg-slate-50/60 dark:bg-slate-800/30" : "bg-white dark:bg-slate-900"} hover:bg-slate-100/70 dark:hover:bg-slate-800/60`}>
                            <td className="px-3 py-2.5 align-top">
                              {renderLevelCell(row, level)}
                            </td>
                            <td className="px-3 py-2.5 text-right font-mono text-[11px] tabular-nums text-slate-900 dark:text-white">{fmt(level.total_pagu)}</td>
                            <td className="px-3 py-2.5 text-right font-mono text-[11px] tabular-nums text-slate-500 dark:text-slate-400">{fmt(level.total_pagu_non_pengadaan)}</td>
                            <td className="px-3 py-2.5 text-right font-mono text-[11px] tabular-nums text-blue-600 dark:text-blue-400">{fmt(level.total_pagu_pengadaan)}</td>
                            <td className="px-3 py-2.5 text-right font-mono text-[11px] tabular-nums text-amber-600 dark:text-amber-400">{fmt(level.total_kebutuhan_anggaran)}</td>
                            <td className="px-3 py-2.5 text-right font-mono text-[11px] tabular-nums text-emerald-600 dark:text-emerald-400">{fmt(level.sisa_pagu_pengadaan)}</td>
                          </tr>
                        )
                      })}
                    </tbody>
                  ...
                  <table className="w-full text-xs whitespace-nowrap">
                    <thead className="bg-slate-50/80 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                      <tr className="text-[10px] uppercase text-slate-400">
                        <th className="font-semibold px-3 py-2 text-center w-10">Pilih</th>
                        <th className="font-semibold px-3 py-2 text-left min-w-[110px]">Rekening</th>
                        <th className="font-semibold px-3 py-2 text-left min-w-[180px]">Nama Standar Harga</th>
                        <th className="font-semibold px-3 py-2 text-right min-w-[110px]">Pagu SIPD</th>
                        <th className="font-semibold px-3 py-2 text-right min-w-[110px]">Pagu Paket</th>
                        <th className="font-semibold px-3 py-2 text-right min-w-[130px]">Rencana Pagu Paket</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {filteredItems.length === 0 ? (
                        ...
                        filteredItems.map((item) => {
                          ...
                          return (
                            <tr key={item.id_sipd_penetapan} className={`transition-colors ${isSelected ? "bg-blue-50/50 dark:bg-blue-500/5" : "hover:bg-slate-50 dark:hover:bg-slate-800/40"}`}>
                              <td className="px-3 py-2 text-center">
                                <button
                                  type="button"
                                  onClick={() => toggleItem(item)}
                                  title={isSelected ? "Batalkan pilihan" : "Pilih rekening ini & atur pagu"}
                                  className={`h-7 w-7 rounded-lg inline-flex items-center justify-center transition-colors ${isSelected ? "bg-blue-600 text-white shadow-xs" : "bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/30 hover:bg-blue-100 dark:hover:bg-blue-500/20"}`}
                                >
