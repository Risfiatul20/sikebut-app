...
export interface ColumnOption {
  id: string
  label: string
  category: "Utama" | "Info Kepegawaian (JSONB)" | "Sistem"
  defaultVisible: boolean
}

export const AVAILABLE_COLUMNS: ColumnOption[] = [
  { id: "nama", label: "Nama Lengkap", category: "Utama", defaultVisible: true },
  { id: "username", label: "Username", category: "Utama", defaultVisible: true },
  { id: "role", label: "Role / Peran", category: "Utama", defaultVisible: true },
  { id: "skpd", label: "SKPD / Perangkat Daerah", category: "Utama", defaultVisible: true },
  { id: "nip", label: "NIP (info.nip)", category: "Info Kepegawaian (JSONB)", defaultVisible: false },
  { id: "jabatan", label: "Jabatan (info.jabatan)", category: "Info Kepegawaian (JSONB)", defaultVisible: false },
  { id: "pangkat_golongan", label: "Pangkat & Golongan", category: "Info Kepegawaian (JSONB)", defaultVisible: false },
  { id: "sub_kegiatan", label: "Sub Kegiatan", category: "Utama", defaultVisible: false },
  { id: "no_hp", label: "No. HP / Kontak", category: "Info Kepegawaian (JSONB)", defaultVisible: false },
  { id: "created_at", label: "Waktu Dibuat", category: "Sistem", defaultVisible: false },
  { id: "aksi", label: "Aksi", category: "Sistem", defaultVisible: true },
]
...
export function ColumnVisibilityDropdown({
  ...
  // Count active columns (excluding 'aksi')
      ...
      {isOpen && (
        <div className="absolute right-0 mt-1.5 w-72 origin-top-right rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="p-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/60 dark:bg-slate-800/40">
            <div>
              <p className="text-xs font-semibold text-slate-900 dark:text-white">Pilih Kolom Tampil</p>
              <p className="text-[10px] text-slate-500 dark:text-slate-400">Centang untuk menampilkan</p>
            </div>
            <button
              type="button"
              onClick={onResetColumns}
              className="inline-flex items-center gap-1 text-[10px] font-medium text-blue-600 dark:text-blue-400 hover:underline"
              title="Kembalikan ke default (Nama, Username, Role, SKPD)"
            >
          ...
          {/* List Options Grouped */}
          <div className="max-h-72 overflow-y-auto p-2 space-y-3 custom-scrollbar text-xs">
            {(["Utama", "Info Kepegawaian (JSONB)", "Sistem"] as const).map((category) => {
              const items = AVAILABLE_COLUMNS.filter((c) => c.category === category)
