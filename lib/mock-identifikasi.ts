import { SipdItem } from "@/types/sipd"
import { IdentifikasiKebutuhan } from "@/types/identifikasi"

export interface ProgramRef {
  kode_program: string
  nama_program: string
  kode_bidang_urusan: string
  nama_bidang_urusan: string
}

export interface KegiatanRef {
  kode_kegiatan: string
  nama_kegiatan: string
  kode_program: string
}

export interface SubKegiatanRef {
  kode_sub_kegiatan: string
  nama_sub_kegiatan: string
  kode_kegiatan: string
}

export const MOCK_PROGRAMS: ProgramRef[] = [
  { kode_program: "1.03.02", nama_program: "Program Penyelenggaraan Jalan", kode_bidang_urusan: "1.03", nama_bidang_urusan: "Pekerjaan Umum dan Penataan Ruang" },
  { kode_program: "1.03.03", nama_program: "Program Pengelolaan Gedung dan Bangunan", kode_bidang_urusan: "1.03", nama_bidang_urusan: "Pekerjaan Umum dan Penataan Ruang" },
  { kode_program: "1.02.03", nama_program: "Program Sediaan Farmasi dan Alat Kesehatan", kode_bidang_urusan: "1.02", nama_bidang_urusan: "Kesehatan" },
  { kode_program: "1.01.02", nama_program: "Program Pengelolaan Pendidikan Dasar", kode_bidang_urusan: "1.01", nama_bidang_urusan: "Pendidikan" },
  { kode_program: "2.16.02", nama_program: "Program Aplikasi Informatika dan E-Government", kode_bidang_urusan: "2.16", nama_bidang_urusan: "Komunikasi dan Informatika" },
]

export const MOCK_KEGIATAN: KegiatanRef[] = [
  { kode_kegiatan: "1.03.02.1.01", nama_kegiatan: "Penyelenggaraan Jalan Kabupaten/Kota", kode_program: "1.03.02" },
  { kode_kegiatan: "1.03.03.1.01", nama_kegiatan: "Penyelenggaraan Bangunan Gedung", kode_program: "1.03.03" },
  { kode_kegiatan: "1.02.03.1.01", nama_kegiatan: "Pengadaan Sediaan Farmasi dan BMHP", kode_program: "1.02.03" },
  { kode_kegiatan: "1.02.03.1.02", nama_kegiatan: "Pengadaan Alat Kesehatan", kode_program: "1.02.03" },
  { kode_kegiatan: "1.01.02.1.01", nama_kegiatan: "Pengelolaan Pendidikan Sekolah Dasar", kode_program: "1.01.02" },
  { kode_kegiatan: "2.16.02.1.01", nama_kegiatan: "Pengelolaan Pusat Data dan Infrastruktur SPBE", kode_program: "2.16.02" },
]

export const MOCK_SUB_KEGIATAN_LIST: SubKegiatanRef[] = [
  { kode_sub_kegiatan: "1.03.02.1.01.0003", nama_sub_kegiatan: "Rekonstruksi Jalan Ruas Godean - Seyegan", kode_kegiatan: "1.03.02.1.01" },
  { kode_sub_kegiatan: "1.03.02.1.01.0004", nama_sub_kegiatan: "Rehabilitasi Jembatan Kali Bedog", kode_kegiatan: "1.03.02.1.01" },
  { kode_sub_kegiatan: "1.03.03.1.01.0002", nama_sub_kegiatan: "Pembangunan Gedung Kantor Pelayanan Terpadu", kode_kegiatan: "1.03.03.1.01" },
  { kode_sub_kegiatan: "1.02.03.1.01.0002", nama_sub_kegiatan: "Pengadaan Obat Esensial dan Vaksin 25 Puskesmas", kode_kegiatan: "1.02.03.1.01" },
  { kode_sub_kegiatan: "1.02.03.1.02.0001", nama_sub_kegiatan: "Pengadaan USG 4 Dimensi dan Monitor Bedside RSUD", kode_kegiatan: "1.02.03.1.02" },
  { kode_sub_kegiatan: "1.01.02.1.01.0003", nama_sub_kegiatan: "Penyediaan Sarana Prasarana dan Meubilair SD Negeri", kode_kegiatan: "1.01.02.1.01" },
  { kode_sub_kegiatan: "2.16.02.1.01.0001", nama_sub_kegiatan: "Pengembangan Infrastruktur Jaringan & Server Data Center", kode_kegiatan: "2.16.02.1.01" },
]

export function getMockSipdBySubKegiatan(kode_sub_kegiatan: string, items: SipdItem[]): SipdItem[] {
  return items.filter((it) => it.kode_sub_kegiatan === kode_sub_kegiatan)
}

function uid() {
  return Math.random().toString(36).slice(2, 10)
}

export function getInitialFormData(tipe: string) {
  const base = {
    nama_paket: "",
    volume: 0,
    volume_satuan: "Unit" as const,
    pdn: "" as const,
    usaha_kecil: "" as const,
    pra_dpa: "" as const,
    spp_ekonomi: "" as const,
    spp_sosial: "" as const,
    spp_lingkungan: "" as const,
    lokasi: [
      {
        id: uid(),
        provinsi: "",
        provinsiCode: "",
        kabupaten: "",
        kabupatenCode: "",
        kecamatan: "",
        kecamatanCode: "",
        detail: "",
      },
    ] as Array<Record<string, string>>,
    sumber_dana: "",
  }

  switch (tipe) {
    case "Barang":
      return {
        ...base,
        fungsi_kegunaan: "",
        uraian: "",
        spesifikasi: "",
        waktu_pemanfaatan_awal: "",
        waktu_pemanfaatan_akhir: "",
        waktu_pemilihan_awal: "",
        waktu_pemilihan_akhir: "",
        waktu_pelaksanaan_kontrak_awal: "",
        waktu_pelaksanaan_kontrak_akhir: "",
        waktu_pelaksanaan_pekerjaan_awal: "",
        waktu_pelaksanaan_pekerjaan_akhir: "",
        metode_pengadaan: "",
        tersedia_ekatalog: "" as const,
        jumlah_dibutuhkan: 0,
        jumlah_sejenis: 0,
        kondisi_baik: 0,
        kondisi_rusak_ringan: 0,
        kondisi_rusak_berat: 0,
        mudah_pasaran: "" as const,
        produsen: "" as const,
        rkbmd_items: [] as Array<{ id: string; sumber: "pengadaan" | "pemeliharaan"; nama_barang: string; kode_fikasi: string; jumlah: number; satuan: string }>,
        rkbmd_mode: "" as const,
        rkbmd_per_anggaran: [],
        kriteria_barang: [] as string[],
        persyaratan_tkdn: "" as const,
        nilai_tkdn: 0,
        cara_pengiriman: "",
        cara_pengangkutan: "",
        cara_pemasangan: "",
        cara_penimbunan: "",
        cara_operasi: "" as const,
        pelatihan: "" as const,
        spp_lanjutan: [] as string[],
        pengadaan_sejenis: "" as const,
        indikasi_konsolidasi: "" as const,
      }
    case "Konstruksi":
      return {
        ...base,
        fungsi: "",
        target_sasaran: "",
        uraian: "",
        spesifikasi: "",
        waktu_pemanfaatan_awal: "",
        waktu_pemanfaatan_akhir: "",
        waktu_pemilihan_awal: "",
        waktu_pemilihan_akhir: "",
        waktu_pelaksanaan_kontrak_awal: "",
        waktu_pelaksanaan_kontrak_akhir: "",
        waktu_pelaksanaan_pekerjaan_awal: "",
        waktu_pelaksanaan_pekerjaan_akhir: "",
        metode_pengadaan: "",
        tersedia_ekatalog_produk: "" as const,
        tersedia_ekatalog_material: "" as const,
        penggunaan_barang_dn: 0,
        penggunaan_barang_ln: 0,
        prioritas: "",
        studi_kelayakan: "" as const,
        dokumen_ded: "" as const,
        kompleksitas: "",
        kontrak_tahun_jamak: "" as const,
        jumlah_tahun_jamak: 1,
        izin_kontrak_jamak: "" as const,
        nomor_izin_jamak: "",
        usaha_kecil_dapat: "" as const,
        pembebasan_lahan: "" as const,
        luas_lahan: 0,
        izin_pemanfaatan_tanah: "" as const,
        lama_pengurusan_lahan: 0,
        status_pembayaran_ganti_rugi: "" as const,
        jumlah_dibutuhkan: 0,
        jumlah_sejenis: 0,
        kondisi_baik: 0,
        kondisi_rusak_ringan: 0,
        kondisi_rusak_berat: 0,
        rkbmd_items: [] as Array<{ id: string; sumber: "pengadaan" | "pemeliharaan"; nama_barang: string; kode_fikasi: string; jumlah: number; satuan: string }>,
        rkbmd_mode: "" as const,
        rkbmd_per_anggaran: [],
        pengadaan_sejenis: "" as const,
        indikasi_konsolidasi: "" as const,
      }
    case "Jasa Lainnya":
      return {
        ...base,
        kebutuhan_rutin: "" as const,
        fungsi_kegunaan: "",
        kompetensi_teknis: "",
        sumberdaya_dimiliki: "",
        jumlah_pelaku: "" as const,
        uraian: "",
        spesifikasi: "",
        waktu_pemanfaatan_awal: "",
        waktu_pemanfaatan_akhir: "",
        waktu_pemilihan_awal: "",
        waktu_pemilihan_akhir: "",
        waktu_pelaksanaan_kontrak_awal: "",
        waktu_pelaksanaan_kontrak_akhir: "",
        waktu_pelaksanaan_pekerjaan_awal: "",
        waktu_pelaksanaan_pekerjaan_akhir: "",
        metode_pengadaan: "",
        tersedia_ekatalog: "" as const,
        pengadaan_sejenis: "" as const,
        indikasi_konsolidasi: "" as const,
      }
    case "Konsultansi":
      return {
        ...base,
        target_sasaran: "",
        uraian: "",
        spesifikasi: "",
        jenis_penyedia: "",
        jumlah_pelaku: "" as const,
        waktu_pemanfaatan_awal: "",
        waktu_pemanfaatan_akhir: "",
        waktu_pemilihan_awal: "",
        waktu_pemilihan_akhir: "",
        waktu_pelaksanaan_kontrak_awal: "",
        waktu_pelaksanaan_kontrak_akhir: "",
        waktu_pelaksanaan_pekerjaan_awal: "",
        waktu_pelaksanaan_pekerjaan_akhir: "",
        metode_pengadaan: "",
        tersedia_ekatalog: "" as const,
        pengadaan_sejenis: "" as const,
        indikasi_konsolidasi: "" as const,
      }
    case "Swakelola":
      return {
        ...base,
        uraian_pekerjaan: "",
        spesifikasi_pekerjaan: "",
        tipe_swakelola: "",
        waktu_awal: "",
        waktu_akhir: "",
      }
    default:
      return {}
  }
}

export const INITIAL_IDENTIFIKASI_LIST: IdentifikasiKebutuhan[] = [
  {
    id: 1,
    user_id: 12,
    pembuat: {
      id: 12,
      nama: "Budi Santoso, S.STP",
      username: "ppk_umum",
    },
    nama_user: "Budi Santoso, S.STP",
    kode_skpd: "1.01.0.00.0.00.01.0000",
    nama_skpd: "DINAS PENDIDIKAN DAN KEBUDAYAAN",
    kode_program: "1.01.02",
    nama_program: "Program Pendidikan Menengah",
    kode_kegiatan: "1.01.02.1.01",
    nama_kegiatan: "Penyelenggaraan Pendidikan Menengah",
    kode_sub_kegiatan: "1.01.02.1.01.0012",
    nama_sub_kegiatan: "Pengadaan Mebel Sekolah",
    nama_paket: "Pengadaan ATK dan Kertas Kantor",
    jenis_pengadaan: "Barang",
    cara_pengadaan: "Penyedia",
    status_review: "Draft",
    total_pagu: "25000000.00",
    jumlah_anggaran: 1,
    waktu_pemanfaatan_awal: "2026-01-05",
    waktu_pemanfaatan_akhir: "2026-01-20",
    waktu_pemilihan_awal: "2026-02-01",
    waktu_pemilihan_akhir: "2026-02-15",
    waktu_pelaksanaan_kontrak_awal: "2026-03-01",
    waktu_pelaksanaan_kontrak_akhir: "2026-03-10",
    waktu_pelaksanaan_pekerjaan_awal: "2026-03-11",
    waktu_pelaksanaan_pekerjaan_akhir: "2026-09-11",
    form_data: {
      nama_paket: "Pengadaan ATK dan Kertas Kantor",
      uraian: "Kebutuhan kertas dan ATK operasional",
      spesifikasi: "Kertas HVS A4 80gr Sinar Dunia",
      volume: 100,
      volume_satuan: "Paket",
      metode_pengadaan: "ePurchasing",
      sumber_dana: "PAD",
    },
    catatan_reviewer: null,
    created_at: "2026-09-02T00:10:00.000000Z",
    updated_at: "2026-09-02T00:10:00.000000Z",
    anggaran: [
      {
        id: 1,
        identifikasi_kebutuhan_id: 1,
        id_sipd_penetapan: 451,
        kode_standar_harga: "SSH-2026-001",
        pagu: "25000000.00",
        standar_harga: {
          kode_standar_harga: "SSH-2026-001",
          nama_standar_harga: "Kertas HVS A4 80gr Sinar Dunia",
        },
      },
    ],
  },
  {
    id: 2,
    user_id: 3,
    pembuat: {
      id: 3,
      nama: "Bambang Sudibyo, S.T., M.M.",
      username: "bambang.ppk",
    },
    nama_user: "Bambang Sudibyo, S.T., M.M.",
    kode_skpd: "1.03.0.00.0.00.01.0000",
    nama_skpd: "Dinas Pekerjaan Umum, Perumahan dan Kawasan Permukiman",
    kode_program: "1.03.02",
    nama_program: "Program Penyelenggaraan Jalan",
    kode_kegiatan: "1.03.02.1.01",
    nama_kegiatan: "Penyelenggaraan Jalan Kabupaten/Kota",
    kode_sub_kegiatan: "1.03.02.1.01.0003",
    nama_sub_kegiatan: "Rekonstruksi Jalan Ruas Godean - Seyegan",
    nama_paket: "Pekerjaan Overlay Aspal Hotmix AC-WC Ruas Godean",
    jenis_pengadaan: "Konstruksi",
    cara_pengadaan: "Penyedia",
    status_review: "Diajukan",
    total_pagu: "12500000000.00",
    jumlah_anggaran: 1,
    waktu_pemanfaatan_awal: "2026-04-01",
    waktu_pemanfaatan_akhir: "2026-11-30",
    waktu_pemilihan_awal: "2026-02-15",
    waktu_pemilihan_akhir: "2026-03-15",
    waktu_pelaksanaan_kontrak_awal: "2026-04-01",
    waktu_pelaksanaan_kontrak_akhir: "2026-10-31",
    waktu_pelaksanaan_pekerjaan_awal: "2026-04-01",
    waktu_pelaksanaan_pekerjaan_akhir: "2026-10-31",
    form_data: {
      nama_paket: "Pekerjaan Overlay Aspal Hotmix AC-WC Ruas Godean",
      uraian: "Rekonstruksi badan jalan dengan aspal hotmix",
      spesifikasi: "Tebal aspal 6cm, lebar jalan 6m",
      volume: 1,
      volume_satuan: "Paket",
      metode_pengadaan: "Tender",
      sumber_dana: "DAK-FISIK",
    },
    catatan_reviewer: null,
    created_at: "2026-08-30T10:00:00.000000Z",
    updated_at: "2026-08-30T10:00:00.000000Z",
    anggaran: [
      {
        id: 2,
        identifikasi_kebutuhan_id: 2,
        id_sipd_penetapan: 1,
        kode_standar_harga: "ASB-2026-0004",
        pagu: "12500000000.00",
        standar_harga: {
          kode_standar_harga: "ASB-2026-0004",
          nama_standar_harga: "Pekerjaan Rekonstruksi & Overlay Aspal Hotmix AC-WC",
        },
      },
    ],
  },
  {
    id: 3,
    user_id: 11,
    pembuat: {
      id: 11,
      nama: "Dewi Lestari, S.Farm., Apt.",
      username: "dewi.ppk",
    },
    nama_user: "Dewi Lestari, S.Farm., Apt.",
    kode_skpd: "1.02.0.00.0.00.01.0000",
    nama_skpd: "Dinas Kesehatan",
    kode_program: "1.02.03",
    nama_program: "Program Sediaan Farmasi dan Alat Kesehatan",
    kode_kegiatan: "1.02.03.1.01",
    nama_kegiatan: "Pengadaan Sediaan Farmasi dan BMHP Puskesmas",
    kode_sub_kegiatan: "1.02.03.1.01.0002",
    nama_sub_kegiatan: "Pengadaan Obat Esensial dan Vaksin 25 Puskesmas",
    nama_paket: "Paket Obat Esensial Generik Puskesmas Se-Kabupaten",
    jenis_pengadaan: "Barang",
    cara_pengadaan: "Penyedia",
    status_review: "Disetujui",
    total_pagu: "5400000000.00",
    jumlah_anggaran: 1,
    waktu_pemanfaatan_awal: "2026-01-10",
    waktu_pemanfaatan_akhir: "2026-12-31",
    waktu_pemilihan_awal: "2026-01-15",
    waktu_pemilihan_akhir: "2026-02-15",
    waktu_pelaksanaan_kontrak_awal: "2026-02-20",
    waktu_pelaksanaan_kontrak_akhir: "2026-12-20",
    waktu_pelaksanaan_pekerjaan_awal: "2026-02-20",
    waktu_pelaksanaan_pekerjaan_akhir: "2026-12-20",
    form_data: {
      nama_paket: "Paket Obat Esensial Generik Puskesmas Se-Kabupaten",
      uraian: "Penyediaan obat rutin puskesmas",
      spesifikasi: "Obat generik terdaftar BPOM dan e-Katalog",
      volume: 1,
      volume_satuan: "Paket",
      metode_pengadaan: "ePurchasing",
      sumber_dana: "DAK-NONFISIK",
    },
    catatan_reviewer: "Disetujui sesuai pagu DAK Non Fisik.",
    created_at: "2026-08-28T08:00:00.000000Z",
    updated_at: "2026-08-29T14:00:00.000000Z",
    anggaran: [
      {
        id: 3,
        identifikasi_kebutuhan_id: 3,
        id_sipd_penetapan: 4,
        kode_standar_harga: "SSH-2026-0812",
        pagu: "5400000000.00",
        standar_harga: {
          kode_standar_harga: "SSH-2026-0812",
          nama_standar_harga: "Paket Obat Esensial Generik Berlogo Sesuai E-Katalog",
        },
      },
    ],
  },
]
